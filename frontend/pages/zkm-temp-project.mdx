...
<Cards>
  <Card title="Project Template" href="https://github.com/zkMIPS/zkm-project-template" arrow />
</Cards>
...
flowchart TD
    subgraph Flow
        A[Guest Program] -->|ELF input| B[Host Program]
        B -->|Calling| C[ZKM SDK]
    end
    
    subgraph Proving Options
        C --> D[Local Proving]
        C --> E[Network Proving]
    end
    
    subgraph Output
        D & E --> F[Proof Contract]
    end
```
## Proving Options
...
<Callout type="info">
> You can run the guest program without generating a proof by setting the environmental variable `EXECUTE_ONLY` to "true". See [issue #152](https://github.com/zkMIPS/zkm/issues/152)
</Callout>
